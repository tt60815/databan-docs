...
| 功能 | URL | 說明 |
|------|-----|------|
| **交易 vs 回測對比** | [/trading/comparison](http://35.236.189.25:8080/trading/comparison) | 按策略顯示實際交易統計 vs 回測統計（勝率、EV、盈虧比） |
| **A↔B 逐筆交易差異** | [/trading/diff](http://35.236.189.25:8080/trading/diff) | 用 DB 1m 資料模擬回測，逐筆比對進出場時間和價格 |
...
- 選擇策略 → 點「開始比對」
- 模擬回測用全部歷史暖機 + 完整狀態機
- 顯示：已匹配、漏單（模擬有實際無）、多單（實際有模擬無）
- 注意：模擬與實際可能有 1~20 分鐘時間差（聚合器暖機差異），屬正常範圍

### 已知限制

- diff 的模擬回測是簡化版狀態機，不完全等同策略引擎的完整邏輯
- 改版後 DB 為唯一真相來源，comparison 的對比更有參考價值
- 未來計畫：將完整策略引擎邏輯移植到 diff，實現 100% 精確重現
